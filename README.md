...
**Four renderer tabs — CSS, Canvas 2D, SVG, WebGPU — draw the same two boxes from the same live state object.** Only the boxes are per-renderer; interaction, construction geometry and lettering live in a shared SVG chrome above them, so the tabs have functional and visual parity by construction, and switching carries the current state with it (also encoded in the URL as `?mode=`). The CSS tab is the honest one: two plain divs where the derived box's `border-radius` is literally the `max()/calc()` expression — the browser's CSS engine does the math. The WebGPU tab is powered by the **Slug algorithm** — Eric Lengyel dedicated the Slug patent to the public domain in March 2026, with MIT reference shaders on GitHub — each box stroke becomes a nonzero-winding ring of quadratic Bézier curves and the fragment shader computes coverage per pixel straight from the curves (no tessellation, no atlases; full Slug adds band acceleration which these curve counts don't need). The typed GPU plumbing (schemas, buffers, bind groups) is [TypeGPU](https://github.com/software-mansion/TypeGPU), lazy-loaded so the base bundle stays small; the tab disables itself where WebGPU is unavailable. A readback harness (`gpu-verify.html` under `vite dev`) pixel-verifies the rings for all three corner shapes.
...
- Drag on any edge or anywhere in the padding area to set the padding — it follows the pointer's depth orthogonal to the grabbed edge, with the same corner construction shown as radius drags. Square corner handles resize (centered); double-click a round handle for sharp corners.
- The second header row picks the corner shape: every CSS `corner-shape` type (round, squircle, bevel, scoop, notch, plus parametric superellipse(k) with a slider) and a true catenary. The superellipse family maps 1:1 onto CSS `corner-shape`; the catenary — not a superellipse — is drawn in the CSS tab as an evenodd `clip-path: shape()` ring over the whole outline where supported, approximated by `superellipse(1.171)` otherwise.
